// Standalone copy of src/data/knowledge.ts — the server runs as a separate
// Node process from the Vite-bundled frontend, so it can't share an ES
// module import across the two without a shared package. Duplicated
// deliberately to keep this "simple backend for testing" a single file with
// no build step of its own; if the knowledge base grows, promote this to a
// shared workspace package instead of hand-syncing two copies.
...
export function answerFor(message: string): { text: string; schedule?: ScheduleItem[] } {
